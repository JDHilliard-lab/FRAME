...
function deleteElevation(idx, e) {
    ...
    if(confirm("Delete this entire elevation wall? This cannot be undone.")) {
        ...
        if (elevations.length === 0) {
            elevations.push({ name: "Elevation 1", frames: [], wallW: 185, wallH: 108, personPos: {x: -60} });
            currentElevIndex = 0;
...
function addNewElevationTab() {
    ...
    elevations.push({ name: "Elevation " + (newIndex + 1), frames: [], wallW: 185, wallH: 108, personPos: {x: -60} });
    renderNavTabs();
    populateDashPushSelector();
...
function syncDashLibraryFolder(e) {
    ...
    for(let file of f) {
        ...
        // Parse the webkitRelativePath - format: "Frame library/Vendor/Collection/file.png"
        // or any depth: root/vendor/collection/file.png
        const parts = file.webkitRelativePath.split('/');
        const filename = parts[parts.length - 1]; // last part is always filename
        
        // Determine vendor and collection from path depth
        // parts[0] = root folder (e.g. "Frame library")
        // parts[1] = vendor (e.g. "Michaelangelo")  
        // parts[2] = collection (e.g. "Avant-Garde")
        // parts[last] = filename
...
function drawElevGuides(wallW, wallH) {
    ...
    createElevArchDim(0, wallH + 3, wallW, wallH + 3, 'h', `${elevFmt(wallW)} ${elevUnit}`, archLayer, true);
    createElevArchDim(-5, 0, -5, wallH, 'v', `${elevFmt(wallH)} ${elevUnit}`, archLayer, true);
}
...
function createElevArchDim(x1, y1, x2, y2, type, label, container, useTicks) {
    const dim = document.createElement('div'); dim.className = 'arch-dim';
    const leftTick = useTicks ? `<div class="tick" style="left:0; ${type==='h'?'top:-6px;':''}"></div>` : '';
    const rightTick = useTicks ? `<div class="tick" style="right:0; ${type==='h'?'top:-6px;':''}"></div>` : '';
    const topTick = useTicks ? `<div class="tick" style="top:0; ${type==='v'?'left:-6px;':''}"></div>` : '';
    const botTick = useTicks ? `<div class="tick" style="bottom:0; ${type==='v'?'left:-6px;':''}"></div>` : '';

    if(type === 'h') {
        const width = Math.abs(x2 - x1) * elevScale; const left = Math.min(x1, x2) * elevScale; const bottom = y1 * elevScale;
        dim.style.cssText = `width:${width}px; height:1.5px; left:${left}px; bottom:${bottom}px;`;
        dim.innerHTML = `${leftTick}${rightTick}<span class="arch-label">${label}</span>`;
    } else {
        const height = Math.abs(y2 - y1) * elevScale; const left = x1 * elevScale; const bottom = Math.min(y1, y2) * elevScale;
        dim.style.cssText = `height:${height}px; width:1.5px; left:${left}px; bottom:${bottom}px;`;
        dim.innerHTML = `${topTick}${botTick}<span class="arch-label" style="${useTicks ? 'transform:rotate(-90deg); left:-15px;' : ''}">${label}</span>`;
    }
...
function createElevArchSpacing(x1, y1, x2, y2, type, container, label) {
    ...
    if(type === 'h') {
        ...
        dim.innerHTML = `<span class="arch-label">${label}</span>`;
    } else {
        const height = Math.abs(y2 - y1) * elevScale; const left = x1 * elevScale; const bottom = Math.min(y1, y2) * elevScale;
...
function setElevUnit(u) {
    ...
    // Save current wall values before converting
...
async function exportElevPNG() {
    ...
    // Strip Art Area completely transparent
    ...
    const oldWallBg = wall.style.background; wall.style.background = 'transparent'; // Kill Wall Fill
    
    const canvas = await html2canvas(wrap, { backgroundColor: null, scale: 2 });
    
    ws.style.overflow = oldOverflow;
    wall.style.background = oldWallBg; // Restore Wall Fill
    arts.forEach((art, i) => { art.style.background = originalBackgrounds[i]; art.style.color = originalColors[i]; }); // Restore Art
